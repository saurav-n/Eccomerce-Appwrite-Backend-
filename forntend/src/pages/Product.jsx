...
import { fetchItems, fetchItemsUser } from "@/app/itemSlice";
...
export default function Product() {
  ...
  useEffect(() => {
    dispatch(fetchItemsUser({ categoryName: category }));
  ...
  useEffect(() => {
    setSuggestions(
      data?.user
        ? data.user.searchHistory.filter((searchedText) =>
    ...
    const updateSearchHistory = async () => {
      try {
        if (
          data &&
          searchedVal &&
          !data.user.searchHistory.includes(
            searchedVal[0].toUpperCase() + searchedVal.slice(1)
          )
        ) {
          console.log("updating search history");
          const response = await axios.post(
            `${import.meta.env.VITE_BACKEND_URL}/api/user/updateSearchHistory`,
            {
              searchedVal,
            },
            {
  ...
  }, [selectedSortType, isOrderedAscendingly]);
      ...
      <div className="w-full h-full py-10 px-2 flex justify-center items-center">
        <div className="w-full max-w-[1000px] flex justify-center">
          ...
          <div className="flex flex-col gap-4 items-center justify-center py-2">
            <div className="flex flex-col gap-y-3 w-full h-[600px] overflow-x-scroll">
              <div className="flex justify-between gap-y-2 flex-wrap">
                <div className="w-full max-w-[150px] relative">
                  <div className="w-full flex gap-x-1 justify-center items-center border-2 border-gray-200 rounded-md py-1 px-2">
                    <input
                  ...
                  <div
                    className={`absolute w-full border-t-[1px] text-sm p-1 rounded-b-md shadow-md z-[98] 
                            bg-white ${
                              isSearchBarFocused ? "inline-block" : "hidden"
                            }`}
                  >
                    <h4>
                      {data && data.user.searchHistory.length
                        ? "Suggestions"
                        : "No Suggestions"}
                    </h4>
                    <div
                    ...
                    >
                      {suggestions.map((suggestion, index) => (
                        <button
                          className={`w-full rounded-md py-1 hover:bg-gray-100 text-left px-1 
                                                text-gray-500 ${
                                                  activeSuggestionIndex ===
                                                  index
                                                    ? "bg-gray-100"
                ...
                <div className="flex gap-x-2 items-center h-fit">
                  <div className="sm:hidden">
                    <Dialog>
                      ...
                      <DialogContent className="w-full max-w-[300px] mt-10 mx-1">
                        <DialogHeader>
                          <DialogTitle>Add Filter</DialogTitle>
                          <DialogDescription>
                            Filters help you finding easily the item you are
                            looking for.
                          </DialogDescription>
                        </DialogHeader>
                        <FilterBox
                  ...
                  <div className="hidden sm:flex gap-x-1 ">
                    <button
                      className={`p-1 bg-gray-300 rounded-md ${
                        orderOfItems === "Grid" ? "text-black" : "text-white"
                      } h-fit`}
                    ...
                    <button
                      className={`p-1 bg-gray-300 rounded-md ${
                        orderOfItems === "List" ? "text-black" : "text-white"
                      } h-fit`}
              ...
              <div
                className={`${
                  orderOfItems === "List"
                    ? "flex flex-col gap-x-2 gap-y-2 flex-wrap"
                    : "grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-4 place-items-center"
                } `}
              >
                {isItemsLoading ? (
                  <Loader />
